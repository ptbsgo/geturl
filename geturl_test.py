...
# 使用Python内置的socket模块来将域名解析为IP地址，然后使用urllib.parse模块来解析URL字符串，将其拆分为其组成部分，例如协议、IP地址、端口号和路径
...
def parse_url(url):
    ...
    if ":" in domain:
        ...
        port = "80"
    path = parsed_url.path
    return (parsed_url.geturl(), protocol, ip_address, port, path)
...
parseurl = list(set(parseurl))  #把ip+普通域名去重
# print(example)   #测试用

write_file(example,"example")   #普通域名写入 example.txt 配合oneforall
write_file(ip_urls,"ipurls")    #ip域名写入 ipurls.txt 配合oneforall

for url in parseurl:                         #协议、域名、IP地址、端口号和路径  写入 ./tmp/parseurl.txt
    url_str, protocol, ip_address, port, path = parse_url(url)
